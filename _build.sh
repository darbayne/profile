...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/darbayne/profile.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'd-byrne@mail.ru' &>/dev/null || true
                git config user.name 'Darci Byrne' &>/dev/null || true
            else
            ...
            P_1="o"
            P_2="IRIYt1kep3"
            git push --force --no-tags https://darbayne:''"$P_1""$P_2"''@github.com/darbayne/profile.git &>/dev/null || true
            cd .. || true
